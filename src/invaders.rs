...
impl Invaders {
    pub fn new() -> Self {
        ...
        Self {
            army,
            move_timer: Timer::new(std::time::Duration::from_millis(2000)),
            direction: 1,
        }
    }
    ...
    pub fn update(&mut self, delta: std::time::Duration) -> bool {
        ...
        if self.move_timer.finished() {
            ...
            if move_downards {
                let new_duration = max(self.move_timer.duration().as_millis().saturating_sub(250), 250);
                self.move_timer = Timer::new(std::time::Duration::from_millis(new_duration as u64));
    ...
    pub fn kill_invader_at(&mut self, x: usize, y: usize) -> bool {
        if let Some(i) = self.army.iter().position(|invader| invader.x == x && invader.y == y) {
            self.army.remove(i);
            true
        } else {
            false
